...
from application.services.booking_service import check_booking, check_bookings, check_user_bookings, cancel_booking, book_room
from fastapi import APIRouter

booking_router = APIRouter(prefix="/bookings", tags = ["Bookings"])

@booking_router.get("", response_model=list[BookingRead])
async def booking(session : SessionDep):
    try:
...
@booking_router.get("/{booking_id}", response_model=list[BookingRead])
